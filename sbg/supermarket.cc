...
private:
  ...
  std::set<std::string> discountItems = {"item1", "item2"}; // Update with your discount items
  ...
  void displayReceipt(double total)
  {
    ...
    for (const std::string &item : scannedItems)
    {
      std::cout << item << " - $" << std::fixed << std::setprecision(2) << items[item] << "\n";
    }
